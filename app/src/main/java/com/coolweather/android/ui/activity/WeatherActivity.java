...
/*
    ...
    private void requestWeather(String weatherId) {
        ...
        HttpUtil.sendOkhttpReauest(weatherUrl, new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                runOnUiThread(new Runnable() {  //切换到主线程中
                    @Override
                    public void run() {
                        if (weather != null && "ok".equals(weather.status)) {
                            SharedPreferences.Editor editor = PreferenceManager
                                    .getDefaultSharedPreferences(WeatherActivity.this).edit();
                            editor.putString("weather", responseText);//将数据保存到数据库中
                            editor.apply();
    ...
    private void showWeatherInfo(Weather weather) {
        ...
        for (Forecast forecast : weather.forecastList) {
            ...
            forecastLayout.addView(view);//添加到父布局
        }
        if (weather.aqi != null) {
        ...
        weatherLayout.setVisibility(View.VISIBLE);//记得将ScrollVIew设置可见
